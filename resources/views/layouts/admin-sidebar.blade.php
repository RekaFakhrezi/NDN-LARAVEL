        ...
        <aside :class="isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="flex-1 py-6 px-4 space-y-2 overflow-y-auto custom-scrollbar">
                ...
                <a href="{{ route('admin.breaking') }}"
                    class="w-full flex items-center justify-between px-4 py-3 rounded-lg font-medium text-sm transition {{ request()->routeIs('admin.breaking') ? 'bg-white text-[#bd2828] font-bold shadow-sm' : 'text-red-100 hover:bg-red-800/50 hover:text-white' }}">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <header
                ...
                <div class="flex items-center gap-4">
                    @php
                        ...
                        $searchAction = route('admin.published'); // Default fallback search target
                        ...
                        if ($currentRoute && in_array($currentRoute, $searchableRoutes)) {
                            $searchAction = request()->url();
                        }
                    @endphp

                    <form action="{{ $searchAction }}" method="GET" class="hidden md:flex relative w-64">
