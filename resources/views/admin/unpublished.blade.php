<x-admin-sidebar>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl md:text-3xl font-serif font-bold text-gray-900">Berita Diturunkan</h2>
            <p class="text-xs text-gray-500 mt-1">Daftar semua artikel yang ditarik/diturunkan dari publikasi.</p>
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 mb-6 shadow-sm">
        <form action="{{ route('admin.unpublished') }}" method="GET" class="flex flex-col sm:flex-row gap-3 items-center w-full">
            <div class="relative flex-1 w-full">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari berita diturunkan..." class="w-full bg-gray-50 border border-gray-200 rounded-xl py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-[#bd2828] focus:ring-1 focus:ring-[#bd2828] focus:bg-white transition-all">
            ...
            @if(request()->filled('search') || request()->filled('category_id'))
                <a href="{{ route('admin.unpublished') }}" class="text-xs font-bold text-[#bd2828] hover:underline whitespace-nowrap px-2">Reset Filter</a>
            @endif
        </form>
    </div>

    <!-- List Berita Diturunkan (Card Horizontal) -->
    <div class="space-y-4">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-5 md:p-6 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col sm:flex-row gap-5 relative group">
                ...
                <div class="flex-1 min-w-0 ml-8 sm:ml-0">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        @endif
                        <span class="text-xs font-bold text-gray-400">Diturunkan: {{ $article->updated_at->format('d M Y H:i') }}</span>
                    </div>
                ...
                <div class="flex flex-row sm:flex-col gap-2 shrink-0 w-full sm:w-32 justify-end sm:justify-start pt-4 sm:pt-0 border-t sm:border-t-0 sm:border-l border-gray-100 sm:pl-5 ml-8 sm:ml-0">
                    <form action="{{ route('admin.republish', $article->id) }}" method="POST" class="flex-1 sm:flex-none">
                        @csrf
                        <button class="w-full bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-lg text-xs font-bold transition-all shadow-sm" onclick="return confirm('Publikasikan kembali berita ini?')">
                            Publish Ulang
                        </button>
                    </form>
                    ...
                    <form action="{{ route('admin.destroy', $article->id) }}" method="POST" class="flex-1 sm:flex-none">
                        @csrf
                        @method('DELETE')
                        <button class="w-full bg-red-50 hover:bg-red-500 text-red-600 hover:text-white px-3 py-2 rounded-lg text-xs font-bold transition-all border border-red-200 hover:border-red-500" onclick="return confirm('Turunkan ke Tempat Sampah?')">
                            Hapus
                        </button>
                    </form>
                </div>
        ...
        @empty
            <div class="bg-white border-2 border-dashed border-gray-300 rounded-xl p-16 text-center flex flex-col items-center justify-center shadow-sm">
                ...
                <p class="text-gray-500 max-w-sm mx-auto text-sm">Tidak ada berita diturunkan yang cocok dengan filter pencarian saat ini.</p>
            </div>
        @endforelse
    </div>
